...
class BotController:
    ...
    def _analyze_slippage(self, order_book: Dict[str, Any], side: str, qty: float, price: float) -> Dict[str, Any]:
        """
        Analyze potential slippage from order book.
        ...
        try:
            if side.lower() == "buy":
                ...
                orders = order_book.get("a", [])
            else:
                # For sell orders, check bids
                orders = order_book.get("b", [])
            ...
            for order in orders:
                ...
                fill_qty = min(remaining_qty, order_qty)
                total_cost += fill_qty * order_price
                total_qty += fill_qty
                depth_usd += order_qty * order_price
            ...
            estimated_price = total_cost / total_qty
            slippage_pct = abs((estimated_price - price) / price) * 100.0
            
            # Check if slippage is acceptable
            is_acceptable = (
                slippage_pct <= RISK_RULES["max_slippage_pct"] and
                depth_usd >= RISK_RULES["min_order_book_depth_usd"]
            )
            ...
            self.log(f"_analyze_slippage error: {e}")
            return {
                "estimated_price": price,
                "slippage_pct": 0.0,
                "depth_usd": 0.0,
    ...
    # ------------------ Order placement with validation and slippage protection ------------------
    def _place_market_order(
        ...
        In dry-run mode, simulates the order using real market data.
        """
        now = now_iso()

        # Get order book for slippage analysis
        order_book = self._get_order_book(client, symbol)
        if order_book:
            slippage_analysis = self._analyze_slippage(order_book, side, qty, price_hint or 0.0)
            
            if not slippage_analysis["is_acceptable"]:
                return {
                    "error": f"Slippage too high: {slippage_analysis['slippage_pct']:.2f}% (max: {RISK_RULES['max_slippage_pct']}%) or insufficient depth: ${slippage_analysis['depth_usd']:.2f} (min: ${RISK_RULES['min_order_book_depth_usd']})",
                    "slippage_analysis": slippage_analysis
                }
            
            # Use estimated price from order book
            estimated_price = slippage_analysis["estimated_price"]
        else:
            estimated_price = price_hint
            slippage_analysis = None

        # Dry-run mode: simulate order with real market data
        if TRADE_SETTINGS.get("dry_run", False):
            self.log(f"[DRY-RUN] Simulating {side} order for {symbol}: qty={qty}, price={estimated_price}")
            return {
                "simulated": True,
                "symbol": symbol,
                "side": side,
        ...
        for name in candidate_methods:
            ...
            for params in params_list:
                try:
                    self.rate_limiter.acquire()
                    resp = meth(**params)
                    self.log(f"{name} succeeded with params {params}")
                    ...
                    if isinstance(resp, dict):
                        ...
                        return {"result": resp, "method": name, "params": params, "slippage_analysis": slippage_analysis}
                    else:
                        return {"result": str(resp), "method": name, "params": params, "slippage_analysis": slippage_analysis}

                except Exception as e:
                    last_exc = e
                    self.log(f"{name} failed with params {params}: {e}")
                    continue
    ...
    def attempt_trade_for_account(self, acct: Dict[str, Any]):
        try:
            ...
            if TRADE_SETTINGS.get("dry_run", False):
                ...
                if account_id not in self.dry_run_balances:
                    # Initialize with a default balance (e.g., $1000)
                    self.dry_run_balances[account_id] = 1000.0
                    self.log(f"[DRY-RUN] Initialized balance for {acct.get('name')}: ${self.dry_run_balances[account_id]:.2f}")
                
                bal = self.dry_run_balances[account_id]
                ok = True
                err = ""
                acct["validated"] = True
                acct["balance"] = bal
                acct["last_validation_error"] = None
            else:
            ...
            # Update dry-run balance
            if TRADE_SETTINGS.get("dry_run", False):
                account_id = acct.get("id")
                self.dry_run_balances[account_id] -= notional
                self.log(f"[DRY-RUN] Updated balance for {acct.get('name')}: ${self.dry_run_balances[account_id]:.2f}")
        ...
        try:
            ...
            if should_close:
                ...
                if TRADE_SETTINGS.get("dry_run", False):
                    ...
                    self.dry_run_balances[account_id] += (entry_price * qty) + profit
                    self.log(f"[DRY-RUN] Trade closed. Profit: ${profit:.2f}. New balance: ${self.dry_run_balances[account_id]:.2f}")
    ...
    def _scan_once(self):
        ...
        for acct in accounts:
            ...
            try:
                # Skip validation in dry-run mode (use simulated balance)
                if not TRADE_SETTINGS.get("dry_run", False):
                    ok, bal, err = self.validate_account(acct)
                    acct["validated"] = ok
                    acct["balance"] = bal
                    acct["last_validation_error"] = err
                    ...
                    if account_id not in self.dry_run_balances:
                        self.dry_run_balances[account_id] = 1000.0
                    acct["validated"] = True
                    acct["balance"] = self.dry_run_balances[account_id]
                    acct["last_validation_error"] = None
            except Exception as e:
